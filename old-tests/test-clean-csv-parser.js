// Test script to validate clean CSV parsing from Hive output
...
function parseHiveOutputToCleanCSV(content) {
  ...
  for (const dataLine of dataLines) {
    ...
    // Check if this is a header line by comparing with known columns
    const values = line
      .split('|')
...
async function run() {
  try {
    ...
    if (result) {
      ...
      const csvPath = path.join(__dirname, 'clean-output.csv');
